...
typedef int (__stdcall *AKP_GetDevStatu)(int,int,int,char*,int*);

DetectDevice g_device;		
DisPatchData g_dispatchData;
CEdit* g_editLog = NULL;
HINSTANCE g_hDllInst;
AKP_GetDevStatu g_deviceStatus;
...
void CTestdllDlg::Ondown() 
{
	...
	g_deviceStatus = (AKP_GetDevStatu)GetProcAddress(g_hDllInst, "AKP_GetDevStatu");

    //判断是否获取成功
	if(g_deviceStatus != NULL)
	{
		//Log("获取dll中的dispatchKeyData方法成功");
	}else{
		Log("获取dll中的dispatchKeyData方法失败");
		return;
	}
